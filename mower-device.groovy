...
def generateEvent(List<Map<String,Object>> updates){
	if (descLog) log.debug "updates: $updates"
			...
			update.each{ String name, value ->
				...
				if(isChange || name in ['id','forced']){
					...
					if(event){
                        ...
                        if (descLog) log.debug "got event : $event, value, $sendValue"
                        def thename = event.name
                        def thevalue = event.value
                        def doUpdate = true
                        if (thename == "mowerActivity")
                          { 
                           // lgk is last was already charging and we get a parked in cs ignore it so we dont get 100 messages
                           def lastActivity = device.currentValue("mowerActivity", true)
                           if (descLog) log.debug "got lastActivity = $lastActivity, new value = $thevalue"
                      
                          if ((lastActivity == "CHARGING") && (thevalue == "PARKED_IN_CS"))
                            {
                             log.warn "Got PARKED_IN_CS when last activity was CHARGING... so ignoring it!" 
                             doUpdate = false
                            }
                          }
                        
					if (doUpdate) sendEvent(event)
